...
#define IDBDATA_SIZE             512
#define SN_SIZE                  30
#define VFLASH_PATH              "/dev/vflash"
#define IO_GET_IDB               0x01


    int fp = 0;
    int ret = -1;
    fp = open(VFLASH_PATH, 0, 0);
    if(fp < 0) {
        ERROR("get idblock, open file fail.");
    } else {
        char idbdata[IDBDATA_SIZE];
        memset(idbdata, 0, sizeof(idbdata));
        ioctl(fp, IO_GET_IDB, idbdata);
        unsigned short size =*((unsigned short *) idbdata);
        if(size > 0 && size <= SN_SIZE) {
            idbdata[size+2] = '\0';
            property_set("ro.serialno", idbdata+2);
            ret = 0;
        }
        close(fp);
    }
    if(ret < 0) {
        ERROR("use default serialno.");
        property_set("ro.serialno", "unknown");
    }
...
int __mount(const char *source, const char *target,
            ...
            const void *options)
{
    if (!strncmp(fstype, "ext3", 4)) {
        int err;
        const char *const resize2fs_argv[] = { "/system/bin/resize2fs", source, NULL };
        /* -y Assume an answer of 'yes' to all questions; allows e2fsck to be used non-interactively. */
        const char *const e2fsck_argv[] = { "/system/bin/e2fsck", "-y", source, NULL };
        /* -j Create the filesystem with an ext3 journal.
           ...
           -q Quiet execution. */
        const char *const mke2fs_argv[] = { "/system/bin/mke2fs", "-j", "-m", "0", "-q", source, NULL };
	...
	if (flags & MS_RDONLY)
        ...
        ERROR("Failed to mount %s, format it now\n", source);
