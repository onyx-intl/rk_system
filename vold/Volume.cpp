...
static const char *stateToStr(int state) {
    ...
    else if (state == Volume::State_Ready)
        return "Ready";    
    else
...
int Volume::formatVol() {
  ...
  //  dev_t partNode = MKDEV(MAJOR(diskNode), 1); // XXX: Hmmm
    ...
    if (!strcmp(getLabel(),"flash")) {
        char cmd[255];
        sprintf(cmd,"mkdosfs /dev/block/vold/%d:%d",
            MAJOR(diskNode), MINOR(diskNode));
       if (system(cmd)<0) {
            SLOGE("Failed to format internal flash dpfdata");
        }
        system("sync");
        setState(Volume::State_Idle);

        memset(cmd,0,255);
        snprintf(cmd, sizeof(cmd), "Volume %s %s partition added (%d:%d)",
                   getLabel(), getMountpoint(), MAJOR(diskNode), MINOR(diskNode));
...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
		...
		if (!strcmp(getLabel(),"flash")){ 
	         ...
	         */ 
	        errno = 0;
	        if (Fat::doMount(devicePath, "/mnt/secure/staging", false, false, 1000, 1015, 0002, true)) {
...
int Volume::unmountVol(bool force) {
    ...
    if ((getState() != Volume::State_Mounted) && (getState() != Volume::State_Ready)) {
