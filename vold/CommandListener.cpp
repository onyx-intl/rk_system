...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
    ...
    int rc = 0;
	 ...
	 if (mDebug) SLOGD("runCommand\n");

    if (!strcmp(argv[1], "list")) {
		 if (mDebug) SLOGD("runCommand %s %s\n", argv[0],argv[1]);
	 ...
	 if (mDebug)	SLOGD("unmount  ar2=%s ar=3%s\n", argv[2],argv[3]);
