...
int VolumeManager::shareVolume(const char *label, const char *method){
    ...
    if ((MAJOR(d) == 0) && (MINOR(d) == 0)) {
        ...
        SLOGE("// This volume does not support raw disk access");
        errno = EINVAL;
        return -1;
    }
    ...
    if(!strcmp(v->getLabel(),"flash")){
       if ((fd = open("/sys/devices/platform/dwc_otg/gadget/lun1/file",
                          O_WRONLY)) < 0) {
               SLOGE("Unable to open ums lun1file (%s)", strerror(errno));
               return -1;
       }
    } else if (!strcmp(v->getLabel(),"sdcard")){
       if ((fd = open("/sys/devices/platform/dwc_otg/gadget/lun0/file",
                          O_WRONLY)) < 0) {
               SLOGE("Unable to open ums lun0file (%s)", strerror(errno));
               return -1;
       }
    }
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    if(!strcmp(v->getLabel(),"flash")){
       ...
       if ((fd = open("/sys/devices/platform/dwc_otg/gadget/lun0/file", O_WRONLY)) < 0) {
           SLOGE("Unable to open ums lun0file (%s)", strerror(errno));
            return -1;
       }
    }
...
int VolumeManager::unmountVolume(const char *label, bool force) 
{
    ...
    if (!v) {
		SLOGW("unmountVolume 111");
        errno = ENOENT;
        return -1;
    }

    if (v->getState() == Volume::State_NoMedia) {
		SLOGW("unmountVolume  2222");
        errno = ENODEV;
        return -1;
    }

    if ((v->getState() != Volume::State_Mounted)   && (v->getState() != Volume::State_Ready)) {
